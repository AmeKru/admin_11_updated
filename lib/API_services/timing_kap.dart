...
class _TimingKAPState extends State<TimingKAP> {
  ...
  Future<void> _fetchBusTimings({
    ...
    // if needed to manually remove item from server (here it would be the one with times = 1)
    // await deleteData('KAP_AfternoonBus', 'times', '1');
    ...
    try {
      ...
      if (kDebugMode) {
        print('Raw data for $infoQueryValue: $data');
      }
      ...
      for (var idx = 0; idx < times.length; idx++) {
        ...
        try {
          ...
          if (timeParts.length == 1) {
            // Single value, interpret as hour only (e.g. "10" -> 10:00)
  ...
  Future<void> getMorningBusKAP() async {
    await _fetchBusTimings(
      infoQueryValue: 'KAP_MorningBus',
      targetList: morningKAP,
    );
  }
  ...
  Future<void> getAfternoonBusKAP() async {
    await _fetchBusTimings(
      infoQueryValue: 'KAP_AfternoonBus',
      targetList: afternoonKAP,
    );
  }
